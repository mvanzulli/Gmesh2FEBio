...
Create virtual environment and install necessary modules with `pip`:
...
# How to run a FE model in FEBio that was meshed in GMSH
    
       
1. The GMsh mesh has to be exported as a <NAME>.msh file in the "VERSION 2 ASCII" format in order to be correctly loaded by FEBio Studio. To do so you should click on:
...
2. In FEBio Studio.
    ...
    ```
    2.2. To run a simulation, at least you must make the following settings [all in the lateral options three]:
...
# ... Runing a FE model in FEBio from console

1. Create a model usig FEBioStudio 

2. Export it as an FE model by doing:

Click on: ` File >> Export FE model ` and save it with a name like <my_FEmodel>.feb.

3. Open a terminal and move to the directory where you save your model and write:

> `febio3`: which allows you to enter in febio.

and then write:

> `run <my_FEmodel>.feb`: and if it is all right, your simulation will start! 
